...
public class VariantUtil {
  // A variant value consists of a one-byte header and a number of content bytes (can be zero). The
  // header byte is divided into upper 6 bits (called "type info") and lower 2 bits (called "basic
  // type"). The `BasicType` enum describes all possibilities of the lower 2 bits.
  ...
  /// Atomic value. The type info value must be one of the values in the below section.
  ...
  // - b4 specifies the type of size. When it is 0/1, `size` is a little-endian 1/3-byte
  // unsigned integer.
  ...
  // Below is all possible type info value for `ATOMIC`.
  ...
  // Long string value. The content is (3-byte little-endian unsigned integer representing the
  // string size) + (size bytes of string content).
  public static final int LONG_STR = 16;

  // At this point, the variant metadata is essentially just a version flag and a dictionary of
  // distinct strings (case-sensitive). Its binary format is:
  // - Version: 1-byte unsigned integer. The only acceptable value is 1 currently.
  // - Dictionary size: 3-byte little-endian unsigned integer. The number of keys in the
  // dictionary.
  // - Offsets: (size + 1) * 3-byte little-endian unsigned integers. `offsets[i]` represents the
  ...
  public static byte atomicHeader(int type) {
    return (byte) (type << 2 | ATOMIC);
  }
